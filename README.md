# NOTE BUZZ ENGINE v3.3.8

note.com記事 + Xスレッド自動生成システム。**3アカウント完全分離**・**海外トレンド自動取得**・**即金体験スタート型レベル進行**・**DALL-E 3 画像自動生成**・**セマンティック重複排除**・**X API自動投稿**・**コスト可視化**・**画像管理・置き換え機能**対応。

## v3.3.8 新機能（画像管理・置き換え機能の完成）

| 機能 | 内容 | 必要キー |
|---|---|---|
| 🎨 画像自動生成 | 記事生成と同時にDALL-E 3でヒーロー画像を作成・記事/X投稿に自動添付。再生成可。 | `OPENAI_API_KEY` |
| 🔄 画像置き換え | 生成画像をローカルファイルでいつでも置き換え可能 | なし |
| 🗑 画像削除 | 不要な画像を削除してテキスト記事に戻す | なし |
| 🖼 複数画像管理 | hero、OGP、Instagram用等複数スロットの画像を独立管理 | なし |
| 📤 ドラッグ&ドロップ | プレビュー領域に直接画像をドラッグして置き換え | なし |
...
### 4. 画像管理・置き換え機能（v3.3.8新機能）
- **🎨 画像再生成**: DALL-E 3で画像を再生成（カスタムプロンプト対応）
- **🔄 画像置き換え**: ローカルの画像ファイルをアップロードして置き換え
- **🗑 画像削除**: 不要な画像を削除
- **📤 ドラッグ&ドロップ**: プレビュー領域に直接画像をドラッグして置き換え
- **🖼 複数スロット管理**: Hero画像、OGP（横長）、Instagram用等複数画像を独立管理
- **💾 自動保存**: アップロードした画像はDBに保存され、記事再表示時に復元

使用方法：
1. 記事を生成後、プレビュー下部の「🎨 画像」セクションから操作
2. 「🔄 画像置き換え」ボタンをクリックしてローカルファイルを選択、または
3. プレビュー領域に直接画像をドラッグ&ドロップ
4. 複数画像（OGPなど）が存在する場合は、各画像に置き換え・削除ボタンあり
...
Renderダッシュボードで New → Web Service → Connect GitHub → **Build**: `npm install && npm rebuild better-sqlite3 && npm run build`（`render.yaml` 利用時と同じ） / **Start**: `node server.js` / **Env**: `ANTHROPIC_API_KEY`、**`NODE_VERSION=20.18.1`** を推奨。
